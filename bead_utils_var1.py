...
drive_idx = 10 ## for data starting 1/27/2024
#drive_idx = 11## for y data 
#drive_idx = 9 ## for z data 
...
def get_noise_template(noise_files, data_idx,nfft=-1, res_pars=[2*np.pi*30, 2*np.pi*5]):
    ...
    for nf in noise_files:
        ...
        if(nfft < 0):
            nfft_to_use = len(cdat[:,data_idx])
        else:
            nfft_to_use = nfft

        cf, cpsd = sp.welch(cdat[:,data_idx], fs=attr['Fsamp'], nperseg=nfft_to_use)
    ...
    resonant_freq=omega0/(2*np.pi)
    print("resonant freq is: %.2f Hz" %resonant_freq)
...
def get_lamp_and_filament(dat, npts):
    ...
    lamp_thresh = 0.5 ## threshold for lamp above which it is on
    filament_thresh = 0.1
    fil_col = 12
    lamp_col = 13
...
def get_noise_template_new(noise_files, data_idx,nfft=-1,res_pars=[2*np.pi*30, 2*np.pi*5],freqs_band=[1,105]):
    ## take noise files and find the average PSD for use in the optimal filter

    J = 0
    nfiles = 0
    for nf in noise_files:
        bandlist=[[0,freqs_band[0]],[freqs_band[1],8100]]
        cdat, attr, _ = get_data(nf)
        ...
        cf, cpsd = sp.welch(cdat[:,data_idx], fs=attr['Fsamp'], nperseg=nfft_to_use)
        fig = plt.figure()
        plt.plot(cf, cpsd,label="ref")
        plt.xscale("log")
        plt.yscale("log")
        plt.ylabel("V/$\sqrt{Hz}$", {'size': 20})
        plt.xlabel("Freq[Hz]", {'size': 20})
        plt.grid(True)
        plt.legend(loc=3)
        plt.show()
        freq_band=np.where((cf>1)&(cf<105))[0]
        peaks=sp.find_peaks(np.sqrt(cpsd[freq_band]),prominence=0.001)
        freqs_to_zero=cf<0 
        print(cf[freq_band][peaks[0]])
        for i in cf[freq_band][peaks[0]]:
            bandlist.append([i-2,i+2])
        for current_band in bandlist:
            freqs_to_zero =  np.logical_or(freqs_to_zero, ( (cf>=current_band[0]) & (cf<=current_band[1]) ))
        cpsd[freqs_to_zero] = 1e20 ## zero out noisy frequencies
        fig = plt.figure()
        plt.plot(cf, cpsd,label="ref")
        plt.xscale("log")
        plt.yscale("log")
...
def signed_correlation_with_step_cal(dat, attr, nperseg=-1, recal_fac = 1/170, use_window=False, drive_idx=drive_idx, has_drive=True):
    ...
    xpsd=np.array(np.abs(np.fft.rfft(xdat[st:en]))**2)
    print(f[didx],xpsd[didx])
    
    plt.figure()
    plt.plot(f,np.abs(np.fft.rfft(ddat[st:en]))**2,label="Drive")
    plt.plot(f,np.abs(np.fft.rfft(xdat[st:en]))**2,label="x Displacement")
    plt.plot(f[didx],xpsd[didx],marker='o', markersize=10, color='blue')
    plt.legend()
    plt.xlim(0,200)
    plt.ylim(0,200)
    plt.show()
    
#     # Time array for length `ddat`
#     t = np.arange(len(ddat))/attr['Fsamp']
#     # Sine wave with frequency `f`
#     ddat = np.sin(2 * np.pi * f[didx] * t)
    ...
    drive_vec = []
    time_out = []
    print(len(np.fft.rfft(xdat[st:en])))
    print(len(np.conjugate(np.fft.rfft(ddat[st:en]))))
    for i in range(nwindows):
        ...
        if(has_drive):
#             cratio = (np.fft.rfft(xdat[st:en]*window)*np.conjugate(np.fft.rfft(ddat[st:en])*window) /max(np.abs(np.fft.rfft(ddat[st:en])*window)))[didx]
            cratio = (np.fft.rfft(xdat[st:en])*np.conjugate(np.fft.rfft(ddat[st:en]))/max(np.abs(np.fft.rfft(ddat[st:en]))))[didx]
...
def get_average_template(calib_dict, data_idx,make_plots=False, fit_pars=[], drive_idx=drive_idx):
    ...
    for impulse_amp in calib_dict.keys():
        ...
        for fname in curr_files:
            ...
            for time_idx in impulse_times:
                ...
                ## make sure the full impulse is contained
                if(time_idx + window_length/2 >= len(cdat[:,data_idx]) or 
                   time_idx < window_length/2):
                    continue
                sidx = int(time_idx-window_length/2)
                eidx = int(time_idx+window_length/2)
                curr_temp += cdat[sidx:eidx,data_idx]
...
def bandpass_filt_0(calib_dict, template_dict, time_offset = 0, bandpass=[], notch_list = [], 
    ...
    return filt_dict
def bandpass_filt(calib_dict,data_idx, template_dict, time_offset = 0, bandpass=[], notch_list = [], 
                  omega0 = 2*np.pi*40, gamma = 2*np.pi*4, subtract_sine_step=False, pulse_data=False, 
    ...
    for impulse_amp in calib_dict.keys():
        ...
        for fname in curr_files:
            ...
            if(make_plots):
                ...
                plt.ylim(0,1.5*np.max(np.abs(corr_vals)*sfac))
                plt.title(impulse_amp)

    return filt_dict

def correlation_filt(calib_dict,data_idx,template_dict, f0=40, time_offset=0, bandpass=[], notch_list = [], 
    ...
    for impulse_amp in calib_dict.keys():
        ...
        for fname in curr_files:
            ...
            if(make_plots):
                ...
                #plt.xlim(0,3e5)
                plt.xlim(0,8e4)
                plt.ylim(0,np.max(np.abs(corr_vals)*sfac))
                plt.title(impulse_amp)

                #plt.figure(figsize=(15,3))
                #fp, psd = sp.welch(cdat[:,data_idx], nperseg=2**16, fs=attr['Fsamp'])
...
def predict_step_impulse(cdat, data_idx,fs, omega0, gamma, make_plots=False):
                xdata = cdat[:,data_idx]
...
def optimal_filt_0(calib_dict, template_dict, noise_dict, pulse_data=True, time_offset=0, 
...
def optimal_filt(calib_dict, data_idx,template_dict, noise_dict, pulse_data=True, time_offset=0, 
                 omega0 = 2*np.pi*40, gamma = 2*np.pi*4, cal_fac=1, drive_idx=drive_idx, 
                 subtract_sine_step=False, make_plots=False):
    ...
    for impulse_amp in calib_dict.keys():
        ...
        for fname in curr_files:
            ...
            if(make_plots):
                ...
                plt.plot(np.abs(corr_data*sfac) )
                plt.plot(corr_idx, np.abs(corr_vals)*sfac, 'ro')
                plt.xlim(740000,750000)
                #plt.xlim(5000, 7500)
                #plt.xlim(impulse_cent[0]-1000, impulse_cent[0]+1000)
                #plt.ylim(0,1.5*np.max(corr_data*sfac))
                plt.ylim(0,0.3*np.max(corr_data*sfac))
                plt.title("opt filt: " + str(impulse_amp) + ", " + fstr)

    return filt_dict, phi_t

def plot_impulse_with_recon_dat(data, attributes, opt_filt, xrange=[-1,-1], cal_facs=[1,1], amp_cal_facs=[1,1], 
                            drive_idx=drive_idx, plot_wind=3, charge_wind=5, charge_range=[-1,-1], 
                            ylim_init=600, plot_wind_zoom=0.5, filt_time_offset = 0,drive_freq=151):

    nyquist =(attributes['Fsamp']/2)
    ## coarse LP filter
    fc_x = np.array([5, 110])/nyquist
    ...
    fc_drive = np.array([drive_freq-1, drive_freq+1])/nyquist
    fc_data = np.array([drive_freq-5, drive_freq+5])/nyquist
    ...
    plt.figure()
#     plt.plot(x_position_data,label='x')
    plt.plot(corr_dat_coarse)
#     plt.plot(corr_dat_fine)
    plt.legend()
    plt.show()
...
def plot_impulse_with_recon(data, attributes, opt_filt, xrange=[-1,-1], cal_facs=[1,1], amp_cal_facs=[1,1], 
    ...
    fc_drive = np.array([drive_freq-1, drive_freq+1])/nyquist
    fc_data = np.array([drive_freq-1, drive_freq+1])/nyquist
    b_data,a_data = sp.butter(3, fc_data, btype='bandpass')
    b_drive,a_drive = sp.butter(3, fc_drive, btype='bandpass')
    tvec = np.arange(len(xdata))/attributes['Fsamp']
